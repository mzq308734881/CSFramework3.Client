...
namespace CSFramework3.Business
{
    ...
    /// 库存调整业务逻辑层
    /// </summary>
    public class bllIA : bllBaseBusiness, IFuzzySearchSupportable
    {
        private IBridge_IA _Bridge = null;

        public bllIA()
        {
            _KeyFieldName = tb_IA.__KeyName;
            _SummaryTableName = tb_IA.__TableName;
            _Bridge = this.CreateBridge();//实例化桥接功能
        }

        private IBridge_IA CreateBridge()
        {
            if (BridgeFactory.BridgeType == BridgeType.ADODirect)
                return new ADODirect_IA().GetInstance();

            if (BridgeFactory.BridgeType == BridgeType.WebService)
                return new WebService_IA();
        ...
        {
            DataTable summaryTable = _CurrentBusiness.Tables[tb_IA.__TableName];
            DataRow row = summaryTable.Rows.Add();
            row[tb_IA.IANO] = "*自动生成*";
            row[tb_IA.DocDate] = DateTime.Today;
            row[tb_IA.DocUser] = Loginer.CurrentUser.Account;
            row[tb_IA.CreatedBy] = Loginer.CurrentUser.Account;
            row[tb_IA.CreationDate] = DateTime.Now;
            row[tb_IA.LastUpdatedBy] = Loginer.CurrentUser.Account;
            row[tb_IA.LastUpdateDate] = DateTime.Now;
        }
        ...
        public override void ApprovalBusiness(DataRow summaryRow)
        {
            ...
            string key = ConvertEx.ToString(summaryRow[tb_IA.IANO]);
        ...
        public override DataSet CreateSaveData(DataSet currentBusiness, UpdateType currentType)
        {
            ...
            DataSet save = this.DoCreateTempData(currentBusiness, tb_IA.__TableName);
            ...
            DataTable detail = currentBusiness.Tables[tb_IAs.__TableName].Copy();
        ...
        {
            string key = this.DataBinder.Rows[0][tb_IA.IANO].ToString();//取单据号码
        ...
        {  //单独处理,即使错误,不向外抛出异常
            ...
            try
            {
                ...
                logBridge.WriteLog(logID, original.Tables[0], changes.Tables[0], tb_IA.__TableName, tb_IA.__KeyName, true); //主表
                logBridge.WriteLog(logID, original.Tables[1], changes.Tables[1], tb_IAs.__TableName, tb_IAs.__KeyName, false);//明细
            }
            catch
            {
