...
namespace CSFramework3.Business
{
    ...
    {
        private IBridge_IO _Bridge = null;

        public bllIO()
        {
            _KeyFieldName = tb_IO.__KeyName;
            _SummaryTableName = tb_IO.__TableName;
            _Bridge = this.CreateBridge();//实例化桥接功能
        }

        private IBridge_IO CreateBridge()
        {
            if (BridgeFactory.BridgeType == BridgeType.ADODirect)
                return new ADODirect_IO().GetInstance();
        ...
        {
            DataTable summaryTable = _CurrentBusiness.Tables[tb_IO.__TableName];
            DataRow row = summaryTable.Rows.Add();
            row[tb_IO.IONO] = "*自动生成*";
            row[tb_IO.DocDate] = DateTime.Today;
            row[tb_IO.DocUser] = Loginer.CurrentUser.Account;
            row[tb_IO.CreatedBy] = Loginer.CurrentUser.Account;
            row[tb_IO.CreationDate] = DateTime.Now;
            row[tb_IO.LastUpdatedBy] = Loginer.CurrentUser.Account;
            row[tb_IO.LastUpdateDate] = DateTime.Now;
        }
        ...
        public override DataSet CreateSaveData(DataSet currentBusiness, UpdateType currentType)
        {
            ...
            DataSet save = this.DoCreateTempData(currentBusiness, tb_IO.__TableName);
            ...
            DataTable detail = currentBusiness.Tables[tb_IOs.__TableName].Copy();
        ...
        {
            string key = this.DataBinder.Rows[0][tb_IO.IONO].ToString();//取单据号码
        ...
        {  //单独处理,即使错误,不向外抛出异常
            ...
            try
            {
                ...
                logBridge.WriteLog(logID, original.Tables[0], changes.Tables[0], tb_IO.__TableName, tb_IO.__KeyName, true); //主表
                logBridge.WriteLog(logID, original.Tables[1], changes.Tables[1], tb_IOs.__TableName, tb_IOs.__KeyName, false);//明细
            }
            catch
            {
