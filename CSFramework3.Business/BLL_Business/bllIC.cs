...
namespace CSFramework3.Business
{
    ...
    {
        private IBridge_IC _Bridge = null;

        public bllIC()
        {
            _KeyFieldName = tb_IC.__KeyName;
            _SummaryTableName = tb_IC.__TableName;
            _Bridge = this.CreateBridge();//实例化桥接功能
        }

        private IBridge_IC CreateBridge()
        {
            if (BridgeFactory.BridgeType == BridgeType.ADODirect)
                return new ADODirect_IC().GetInstance();

            if (BridgeFactory.BridgeType == BridgeType.WebService)
                return new WebService_IC();
        ...
        {
            DataTable summaryTable = _CurrentBusiness.Tables[tb_IC.__TableName];
            DataRow row = summaryTable.Rows.Add();
            row[tb_IC.ICNO] = "*自动生成*";
            row[tb_IC.DocDate] = DateTime.Today;
            row[tb_IC.DocUser] = Loginer.CurrentUser.Account;
            row[tb_IC.CreatedBy] = Loginer.CurrentUser.Account;
            row[tb_IC.CreationDate] = DateTime.Now;
            row[tb_IC.LastUpdatedBy] = Loginer.CurrentUser.Account;
            row[tb_IC.LastUpdateDate] = DateTime.Now;
        }
        ...
        public override void ApprovalBusiness(DataRow summaryRow)
        {
            ...
            string key = ConvertEx.ToString(summaryRow[tb_IC.ICNO]);
        ...
        public override DataSet CreateSaveData(DataSet currentBusiness, UpdateType currentType)
        {
            ...
            DataSet save = this.DoCreateTempData(currentBusiness, tb_IC.__TableName);
            ...
            DataTable detail = currentBusiness.Tables[tb_ICs.__TableName].Copy();
        ...
        {
            string key = this.DataBinder.Rows[0][tb_IC.ICNO].ToString();//取单据号码
