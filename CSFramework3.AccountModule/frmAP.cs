 ...
 *  应付款示例说明
 *  
 *  应付款AP，包含对采购单(tb_SO)及它项(tb_AccountIDs)进行付款。
 * 
 *  它项(tb_AccountIDs)表定义了不同类型的付款项目，在AP的明细表可以直接输入编号进行付款。
 * 
 *  Outstanding/Balance 帐款结余 = 本单的金额 - 本单的已付金额(审核的AP单)
 * 
 ************************************************************************************************************* */
...
namespace CSFramework3.AccountModule
{
    ...
    /// 应付款界面
    /// </summary>
    public partial class frmAP : frmBaseBusinessForm
    {
        ...
        {
            DataRow result = frmQueryOutstanding.Execute_SelectRow(ConvertEx.ToString(txtSupplier.EditValue),OutstandingType.AP);
        ...
        private void OnInvoiceNo_Validating(object sender, CancelEventArgs e)
        {
            ...
            //可能是单据号码
            DataTable dt = CommonData.SearchOutstanding(invoiceNo, "", DateTime.MinValue, DateTime.MinValue, DateTime.MinValue, "AP");
        ...
        public override void DoSave(IButtonInfo sender)// 保存数据
        {
            ...
            if (!ValidatingSummaryData(_BLL.CurrentBusiness.Tables[tb_AP.__TableName])) return; //检查主表数据合法性
            if (!ValidatingDetailData(_BLL.CurrentBusiness.Tables[tb_APs.__TableName])) return; //检查从表数据合法性
            ...
            {
                if (_UpdateType == UpdateType.Add) _BLL.DataBindRow[tb_AP.__KeyName] = result.DocNo; //更新单据号码
        ...
        protected override void ButtonStateChanged(UpdateType currentState)// 按钮状态改变时触发的事件
        {
            ...
            this.SetEditorEnable(txtSupplierName, false, true);//禁用客户名称
        ...
        protected override void DoBindingSummaryEditor(DataTable dataSource)
        {
            ...
            DataBinder.BindingTextEdit(txtAPNO, dataSource, tb_AP.APNO);
            DataBinder.BindingTextEdit(txtCreatedBy, dataSource, tb_AP.CreatedBy);
            DataBinder.BindingTextEdit(txtCreationDate, dataSource, tb_AP.CreationDate);
            DataBinder.BindingTextEdit(txtCurrency, dataSource, tb_AP.Currency);
            DataBinder.BindingTextEdit(txtRemark, dataSource, tb_AP.Remark);
            DataBinder.BindingTextEdit(txtAmount, dataSource, tb_AP.Amount);
            DataBinder.BindingTextEdit(txtSupplier, dataSource, tb_AP.SupplierCode);
            DataBinder.BindingTextEdit(txtSupplierName, dataSource, tb_AP.SupplierName);
            DataBinder.BindingTextEdit(txtChequeNo, dataSource, tb_AP.ChequeNo);
            DataBinder.BindingTextEdit(txtReceivedDate, dataSource, tb_AP.ReceivedDate);
            DataBinder.BindingTextEdit(txtChequeBank, dataSource, tb_AP.ChequeBank);
            DataBinder.BindingTextEdit(txtAppDate, dataSource, tb_AP.AppDate);
            DataBinder.BindingTextEdit(txtAppUser, dataSource, tb_AP.AppUser);
        }
        ...
        protected override void CreateOneDetail(GridView gridView, int buttonIndex)
        {
            ...
            DataRow row = _BLL.CurrentBusiness.Tables[tb_APs.__TableName].NewRow();
            string order = new GenerateSortID().Generate(gvDetail, colD_Queue); //生成排序编号
            row[tb_APs.Queue] = order; //排序编号
            row[tb_APs.Currency] = Globals.DEF_CURRENCY; //预设货币
            if (buttonIndex == DetailButtons.Add)
            {
                _BLL.CurrentBusiness.Tables[tb_APs.__TableName].Rows.Add(row); //增加一条明细记录
            ...
            else if (buttonIndex == DetailButtons.Insert)
            {
                ...
                _BLL.CurrentBusiness.Tables[tb_APs.__TableName].Rows.InsertAt(row, gvDetail.FocusedRowHandle);
        ...
        {
            if (string.IsNullOrEmpty(ConvertEx.ToString(txtSupplier.EditValue)))
            {
                Msg.Warning("客户不能为空!");
                txtSupplier.Focus();
                return false;
        ...
        private bool ValidatingDetailData(DataTable detail)
        {
            ...
            foreach (DataRow row in detail.Rows)
            {
                ...
                docNo = ConvertEx.ToString(row[tb_APs.InvoiceNo]);
        ...
        {
            DataTable dt = (_BLL as bllAP).GetSummaryByParam(txt_DocNoFrom.Text, txt_DocNoTo.Text, txt_DocDateFrom.DateTime, txt_DocDateTo.DateTime);
        ...
        protected override void DoBindingDetailGrid(DataSet dataSource)// 绑定单据的所有明细表. 
        {
            ...
            gcDetail.DataSource = dataSource.Tables[tb_APs.__TableName];
        }

        public override void DoPrint(IButtonInfo button)
        {
            string docNo = this.GetSummaryFieldValue(tb_AP.__KeyName);

            //FastReport DEMO
            CSFramework3.ReportsFastReport.frmReportAP.Execute(docNo, docNo);
        }
