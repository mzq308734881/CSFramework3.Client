...
namespace CSFramework3.Server.DataAccess.DAL_Business
{
    ...
    /// dalAR
    /// </summary>
    public class dalAR : dalBaseBusiness, IBridge_AR
    {
        /// <summary>
        /// 构造器
        /// </summary>
        /// <param name="loginer">当前登录用户</param>
        public dalAR(Loginer loginer)
            : base(loginer)
        {
            _SummaryKeyName = tb_AR.__KeyName; //主表的主键字段
            _SummaryTableName = tb_AR.__TableName;//主表表名
        ...
        {
            Type ORM = null;
            if (tableName == tb_AR.__TableName) ORM = typeof(tb_AR);
            if (tableName == tb_ARs.__TableName) ORM = typeof(tb_ARs);
        ...
        public DataTable GetSummaryByParam(string docNoFrom, string docNoTo, DateTime docDateFrom, DateTime docDateTo)
        {
            ...
            if (docNoFrom != string.Empty) sql.Append(" and " + tb_AR.ARNO + ">='" + docNoFrom + "'");
            if (docNoTo != string.Empty) sql.Append(" and " + tb_AR.ARNO + "<='" + docNoTo + "'");
        ...
        {
            string sql1 = "select a.*,b.NativeName AS CustomerName from tb_AR a LEFT JOIN tb_Customer b ON a.CustomerCode=b.CustomerCode where [ARNO]=@DocNo1 ";
            string sql2 = " select * from [vw_ARs]   where [" + tb_AR.__KeyName + "]=@DocNo2 ORDER BY Queue "; //明细表排序
            ...
            ds.Tables[0].TableName = tb_AR.__TableName;
            ds.Tables[1].TableName = tb_ARs.__TableName;//明细表
            return ds;
        }
        ...
        {
            //删除单据:只删除明细,主表金额清零!!!
            string sql1 = "UPDATE [tb_AR] SET Amount=0 where [" + tb_AR.__KeyName + "]=@DocNo1 ";
            string sql2 = "delete [tb_ARs] where [" + tb_AR.__KeyName + "]=@DocNo2 ";
            SqlCommandBase cmd = SqlBuilder.BuildSqlCommandBase(sql1 + sql2);
            cmd.AddParam("@DocNo1", SqlDbType.VarChar, docNo.Trim());
            cmd.AddParam("@DocNo2", SqlDbType.VarChar, docNo.Trim());
        ...
        {
            string docNo = DocNoTool.GetNumber(tran, "AR");
            return docNo;
        }
        ...
        {
            SqlProcedure sp = SqlBuilder.BuildSqlProcedure("sp_RptAR");
